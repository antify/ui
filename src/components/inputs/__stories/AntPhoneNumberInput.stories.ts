...
export const PlaceholderDemo: Story = {
  render: (args) => ({
    ...
    setup() {
      const phoneMobile = ref(null);
      const countryMobile = ref(null);

      const phoneFixed = ref(null);
      const countryFixed = ref(null);
    ...
    template: `
      <div class="space-y-8 p-4">
        <div class="flex items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-base-200">
          <label class="text-sm font-semibold text-for-white-bg-font">Locale (Language):</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div class="p-6 border border-base-200 rounded-lg bg-base-50">
            <h3 class="mb-3 text-md font-bold text-for-white-bg-font flex items-center gap-2">
              Mobile Phone Mode <code>isMobile={true}</code>
            </h3>

            <p class="text-xs text-base-400 mb-4">
              Used for sending codes (OTP), messenger services, or authentication.
            </p>

            <AntPhoneNumberInput
              v-bind="args"
              v-model="phoneMobile"
              v-model:country-value="countryMobile"
          ...
          <div class="p-6 border border-base-200 rounded-lg bg-base-50">
            <h3 class="mb-3 text-md font-bold text-for-white-bg-font flex items-center gap-2">
              Standard Phone Mode <code>isMobile={false}</code>
            </h3>

            <p class="text-xs text-base-400 mb-4">
              Used for landline phones, clinic contacts, or B2B communication.
            </p>

            <AntPhoneNumberInput
              v-bind="args"
              v-model="phoneFixed"
              v-model:country-value="countryFixed"
              :locale="selectedLocale"
              :is-mobile="false"
            />
          </div>
  ...
  args: {
    label: 'Phone Input (Placeholder Check)',
    countries: COUNTRIES,
    size: Size.md,
  },
};
